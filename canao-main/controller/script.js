...
socket.addEventListener('message', message => {
  ...
  if (msg.type === 'compute-start') {
    ...
    if (history = []){
      history.push(msg.payload.prompt)
    }
  }
  ...
  if (msg.type === 'compute-end') {
    if (msg.payload) {
      ...
      if (msg.context && msg.context.length >= 7){
        msg.context.splice(0,1)
      }
      state.context.push(msg.payload.response)
